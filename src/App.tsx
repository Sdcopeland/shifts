...
import { Calendar, Download, Plus, Trash2, Settings, ChevronLeft, ChevronRight, Moon, Sun, Briefcase, Check, RotateCcw } from 'lucide-react';
...
interface ShiftConfig {
  ...
  iconName: 'Sun' | 'Briefcase' | 'Moon' | 'Check';
}
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      if (savedConfigs) {
        setShiftConfigs(JSON.parse(savedConfigs));
      }
    } catch (e) {
      console.error("Failed to load data", e);
    } finally {
      setIsLoaded(true); // Mark as loaded so we can start saving safely
    }
  }, []);

  // Save shifts whenever they change (only if loaded)
  useEffect(() => {
    if (isLoaded) {
  ...
  const updateConfigTime = (id: ShiftType, field: 'startTime' | 'endTime', value: string) => {
      ...
      <main className="flex-1 overflow-y-auto pb-20">
                  ...
                  >
                    ...
                    {shift && (
                      <div className="w-full flex-1 flex flex-col items-center justify-center gap-1 mt-1">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${shiftConfigs.find(t => t.id === shift.type)?.color || 'bg-gray-100'}`}>
                           {getIcon(shiftConfigs.find(t => t.id === shift.type)?.iconName || 'Briefcase')}
                        </div>
                 ...
                 return (
                   <div key={shift.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4">
                     <div className={`w-12 h-12 rounded-full flex items-center justify-center ${config?.color}`}>
                       <span className={config?.textColor}>{getIcon(config?.iconName || 'Briefcase')}</span>
                     </div>
                     <div>
        ...
        {view === 'settings' && (
          <div className="p-4 space-y-6">
            <h2 className="text-lg font-semibold text-slate-800">Default Shift Times</h2>
            <p className="text-sm text-slate-500">Changes made here will apply to new shifts you create.</p>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex-1">
                      ...
                      <input 
                        ...
                        onChange={(e) => updateConfigTime(config.id, 'startTime', e.target.value)}
                    ...
                    <div className="flex-1">
                      ...
                      <input 
                        ...
                        onChange={(e) => updateConfigTime(config.id, 'endTime', e.target.value)}
                        className="w-full mt-1 p-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 font-medium focus:ring-2 focus:ring-blue-500 focus:outline-none"
                      />
                    </div>
                  </div>
